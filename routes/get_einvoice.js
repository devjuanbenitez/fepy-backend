...
router.post('/get_einvoice', async (req, res) => {
  try {
    ...
    const establecimiento = datosFactura.data?.establecimiento || datosFactura.establecimiento || '001';
    const punto = datosFactura.data?.punto || datosFactura.punto || '001';
    const numero = datosFactura.data?.numero || datosFactura.numero || '0000001';
    const correlativoCompleto = `${establecimiento}${punto}${String(numero).padStart(7, '0')}`;

    const totalFactura = datosFactura.data?.total || datosFactura.totalPago || datosFactura.total ||
